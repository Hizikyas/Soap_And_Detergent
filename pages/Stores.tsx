...
// Dynamically import Leaflet and components with SSR disabled
...
const StoresMapSection: React.FC<{ 
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && L) {
      // Type-safe icon prototype modification
  ...
  if (typeof window === 'undefined') {
    return (
      <div className={`h-96 rounded-lg overflow-hidden shadow-lg ${darkMode ? 'bg-gray-800' : 'bg-gray-200'} flex items-center justify-center`}>
  ...
  return (
    <div className={`h-96 rounded-lg overflow-hidden shadow-lg ${darkMode ? 'bg-gray-800' : 'bg-gray-200'}`}>
      <MapContainer
        center={position}
        zoom={12}
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-[#FCF7F8]'} transition-colors duration-300`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center mb-[3rem]">
          <h1 className={`text-4xl md:text-5xl font-bold ${darkMode ? 'text-white' : 'text-[#A31621]'} relative inline-block`}>
            Stores
            <div className={`absolute -bottom-2 left-1/2 transform -translate-x-1/2 translate-y-1 w-20 h-0.5 ${darkMode ? 'bg-white' : 'bg-[#A31621]'}`}></div>
          </h1>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {stores.map((store, index) => (
            <div
              key={store.id}
              className={`${darkMode ? 'bg-gray-800' : 'bg-white'} p-6 rounded-lg shadow-lg transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 hover:bg-opacity-90 opacity-0 animate-[fadeInUp_0.5s_ease-out_forwards]`}
              style={{ animationDelay: `${index * 100}ms` }}
              onMouseEnter={() => setHoveredStore(store.id)}
              onMouseLeave={() => setHoveredStore(null)}
            >
              <h3 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-[#A31621]'} mb-4`}>
                {store.name}
              </h3>
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  <MapPin className={`${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-1`} size={16} />
                  <span className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    {store.address}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <Phone className={`${darkMode ? 'text-gray-400' : 'text-gray-600'}`} size={16} />
                  <span className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    {store.phone}
                  </span>
                </div>
                <div className="flex items-start gap-3">
                  <Clock className={`${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-1`} size={16} />
                  <span className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    {store.hours}
                  </span>
                </div>
